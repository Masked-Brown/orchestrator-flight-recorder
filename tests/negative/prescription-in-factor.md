...
*NEGATIVE fixture — deliberately broken, and expected to fail the gate on `prescription`.*
*A fix is smuggled into a contributing factor.*
...
2. The transmitted reply named the grid being used but not that a selection had been made; next time the reply needs to surface the choice — msg 6 (assistant, SAID)
