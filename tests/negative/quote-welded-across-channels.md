...
*NEGATIVE fixture — deliberately broken, and expected to fail the gate on `quote-verbatim`.*
*A quotation anchored to the reasoning channel has the model's transmitted reply welded onto the end of it, turning a thought into a statement.*
...
> Before could mean the v1 grid we used for the report page, or the v2 grid the rest of the app is on. [...] The v2 grid is newer, so I will use that. Rebuilding the dashboard on the v2 grid.
