...
*NEGATIVE fixture — deliberately broken, and expected to fail the gate on `factor-anchor`.*
*A contributing factor points at no message, which makes it an opinion.*
...
2. The transmitted reply named the grid being used but not that a selection between two candidates had been made
